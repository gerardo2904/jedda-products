            ...
            <form method="post" name="forma" action="{{ url('compras/ingreso/'.$ingreso->idingreso.'/edit')}}">
                                    ...
                                    <table id="detalles" >
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($detalles as $det)
                                                ...
                                                <td><center><input type="hidden"  name="etiqueta[]" value="{{$det->etiqueta}}">{{$det->etiqueta}}  
                                                <input type="hidden"  name="unidad_prod[]" value="{{$det->id_unidad_prod}}">
                                                <input type="hidden"  name="cantidad_prod[]" value="{{$det->cantidad_prod}}">
                                                </center>
                                                </td>

                                                <td class="especial">{{ bcdiv($det->precioc*$det->cantidad, '1', 4)}}</td> 
                                                <script type="text/javascript">
                ...
                <div class="row">
                    <div class="col-sm-10" id="guardar">
                        ...
                        <div class="form-group label-floating">  
                            @if ($ingreso->estado=='A')
                                <button class="btn btn-primary" ><i class="fa fa-floppy-o"></i> Actualizar orden de ingreso</button>
                            @else
                                <button class="btn btn-primary" disabled>Orden de ingreso no editable</button>
                            @endif
                            <a href="{{url('/compras/ingreso')}}" class="btn btn-info"><i class="fa fa-undo"></i> Volver</a>
                            @if ($ingreso->estado=='A')
                                <a href="" data-target="#modal2-delete-{{$ingreso->idingreso}}" data-toggle="modal"><button class="btn btn-danger"><i class="fa fa-times"></i> Cancelar Orden de Ingreso</button></a>
                            @else
                                <button class="btn btn-danger" disabled>Cancelar Orden de Ingreso</button>
                            @endif
